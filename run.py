...
from flask_server.university.models import Holidays, Course, Student, Teacher, Notices
...
def normal_chat():
    ...
    if tag == "notices":
        ...
        link = f'http://127.0.0.1:5000/notices/download/{notice.id}/'
        response = f"Notice titled '{notice.file_name}' is available for download"
        return jsonify({
            'response': response, 'tag': tag,
            "data": {
